...
import static post.classes.SpongeAdapter.getMatchingStacks as spongeMatchingStacks
...
var setupEmpowerer = { int blockItemMeta, Color particleColor ->
    return mods.actuallyadditions.empowerer.recipeBuilder()
        ...
        .particleColor(particleColor.RGB)
        .energy(1_000_000)
        .time(400)
}
...
setupAndRemoveEmpowerer(0, item('moreplates:empowered_restonia_gear'), Color.RED)
    .input(
        ore('gemAlmandine'),
        ore('ingotRedAlloy'),
        fillMetaItem(fluid('sulfuric_acid'), MetaItems.FLUID_CELL_GLASS_VIAL),
...
setupAndRemoveEmpowerer(1, item('moreplates:empowered_palis_gear'), Color.BLUE)
    .input(
        ...
        fillMetaItem(fluid('water'), MetaItems.FLUID_CELL),
...
setupAndRemoveEmpowerer(2, item('moreplates:empowered_diamatine_gear'), Color.CYAN)
    .input(
        ...
        fillMetaItem(fluid('nitrogen_dioxide'), MetaItems.FLUID_CELL),
...
for (var oilType : ['oil', 'oil_heavy', 'oil_medium', 'oil_light']) {
    setupEmpowerer(3, Color.DARK_GRAY)
        .input(
            ...
            ore('ingotDarkSteel'),
            fillMetaItem(fluid(oilType), MetaItems.FLUID_CELL),
...
setupAndRemoveEmpowerer(4, item('moreplates:empowered_emeradic_gear'), Color.GREEN)
    .input(
        item('nomilabs:stabilizeduranium'),
        ore('ingotVibrantAlloy'),
        fillMetaItem(fluid('argon'), MetaItems.FLUID_CELL),
...
setupAndRemoveEmpowerer(5, item('moreplates:empowered_enori_gear'), Color.WHITE)
    .input(
        ore('gemApatite'),
        ore('ingotEndSteel'),
        fillMetaItem(fluid('helium'), MetaItems.FLUID_CELL),
...
class IngredientMCEmpower extends SimpleIIngredient {
    ...
    @Override
    ItemStack[] getMatchingStacks() { return spongeMatchingStacks(ing) }
